...
void getDataFromFile(Data **data, char * fileName,  int sz) {
  float x, y, z; 
   int data_counter = 0;
  
   *data = (Data*)malloc(sizeof(Data)*sz);//new Data[100];

   std::fstream infile("/home/matthew/Downloads/9727OS_Final_Code/B02489_03_Code/code/gaussian/Points.txt",std::ios_base::in);
   //printf("no");
   //printf("%i",sz);
  while(infile >> x >> y >> z)
    {
      // printf("wtf");
      (*data)[data_counter].x =  (float)x/100;
      (*data)[data_counter].y = (float)y/100;//2.0f*y;
      (*data)[data_counter].z = (float)z/100;
      //printf("%f",(*data)[data_counter].z);
      data_counter++;
    }
...
void gaussianDemo(float sigma){
  ...
  char fileName [] = "/home/matthew/Downloads/9727OS_Final_Code/B02489_03_Code/code/gaussian/Points.txt";
  //printf("hi");
  std::ifstream infile("/home/matthew/Downloads/9727OS_Final_Code/B02489_03_Code/code/gaussian/Points.txt");	
  int sz = 0;
  ...
  static Data *data = NULL;//=(Data*)malloc(sizeof(Data)*num_points);
  ...
  */
  ...
  //if(!data)
  getDataFromFile(&data,fileName,sz);
  draw2DHeatMap(data, sz);
  //delete []data;
  free(data);
...
int main(void)
{
	...
	window = glfwCreateWindow(WINDOWS_WIDTH, WINDOWS_HEIGHT, "Viz Points", NULL, NULL);
	if (!window){
		glfwTerminate();
		exit(EXIT_FAILURE);
	}
